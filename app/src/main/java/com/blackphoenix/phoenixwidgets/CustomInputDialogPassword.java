...
public abstract class CustomInputDialogPassword extends Dialog {

    private Context dn_dialogContext;
    private static String LOG_TITLE = CustomInputDialogPassword.class.getSimpleName();
    private String defaultName = null;
    private boolean isMobileNumberValid = false;

    public CustomInputDialogPassword(Context context, String name){
        this(context);
        this.defaultName = name;
    }

    protected CustomInputDialogPassword(Context context) {
        this(context,  R.style.PxwCustomDialogTheme);
        dn_dialogContext = context;
    }

    private CustomInputDialogPassword(Context context, int themeResId) {
    ...
    public void onCreate(Bundle savedInstanceState){
        ...
        setContentView(R.layout.pxw_dialog_input_password);
        ...
        dds_inputName.addTextChangedListener(new InputTextChangeListener() {
            ...
            public void validateText(String data) {
                String USERNAME_PATTERN = "[a-zA-Z0-9]{3,8}";
                Pattern pattern = Pattern.compile(USERNAME_PATTERN);
                Matcher matcher = pattern.matcher(data);
                if(!matcher.matches()){
                    dds_inputName.setError("Password must contain alphabets/numbers and size 3-8");
        ...
        dds_buttonCreate.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if(!_isNameAvailable){
                    Toast.makeText(dn_dialogContext,"Enter Valid Password",Toast.LENGTH_SHORT).show();
                } else {
    ...
    public abstract void onPasswordReady(String password);
}
